...
├── api/                       ← 5 funciones de servidor (límite del plan gratis de Vercel: 12)
│   ├── wompi-sign.js          ← firma segura para el pago
│   ├── wompi-webhook.js       ← recibe la confirmación de pago de Wompi
│   ├── check-availability.js  ← consulta disponibilidad del calendario
│   ├── photos-public.js       ← fotos del alojamiento (para la guía)
│   └── admin.js               ← TODO el panel de admin consolidado en una función
├── lib/                       ← código compartido (no cuenta como funciones)
│   └── (varios archivos _*.js con lógica reutilizada por las funciones de arriba)
...
> ⚠️ **Por qué está así de consolidado:** el plan gratuito (Hobby) de Vercel
> permite un máximo de **12 Serverless Functions** por proyecto. Cada
> archivo `.js` dentro de `api/` cuenta como una función — por eso todo el
> panel de administración (que antes eran 10 archivos separados) ahora
> vive en un solo `api/admin.js`, y el código que comparten varias
> funciones se movió a `lib/` (esa carpeta no cuenta para el límite).
...
2. Adentro deberías ver: `index.html`, `admin.html`, `README.md`, las carpetas `api`, `lib`, `assets`
3. **Selecciona todos esos elementos** (haz clic en el primero, mantén `Shift` y haz clic en el último para seleccionar todos)
4. En GitHub, en tu repositorio nuevo, busca el link **"uploading an existing file"**
5. **Arrastra todos los elementos seleccionados** (no la carpeta `guest-guide-2026` completa) hacia el recuadro punteado
...
## 📸 Gestionar las fotos del alojamiento desde el panel

Ahora puedes agregar, quitar y reordenar las fotos que ve el huésped en la
guía, directamente desde `admin.html` — sin tocar código ni volver a
desplegar.

### Configuración (una sola vez)

1. En Supabase, ve a **"Storage"** (menú izquierdo) → **"New bucket"**
2. Nombre exacto: `property-photos`
3. Actívalo como **"Public bucket"** (para que las fotos se puedan ver en
   la guía sin necesitar contraseña)
...
5. Sube a GitHub los archivos nuevos: `lib/_storage.js` y el `api/admin.js`
   actualizado (ya incluye las acciones de fotos)

No hace falta ninguna variable de entorno nueva — reutiliza las mismas
llaves de Supabase que ya tenías.

### Cómo funciona

- En `admin.html` hay una nueva sección **"Fotos del alojamiento"**: sube
  una foto nueva con el botón "+ Agregar foto" (se redimensiona sola en
  tu navegador antes de subirla, no te preocupes por el tamaño del
  archivo), reordénalas con las flechas ↑ ↓, o bórralas con ✕
- La **primera foto de la lista** es la que aparece primero en la galería
  de la guía
- **Mientras no subas ninguna foto por aquí**, la guía sigue mostrando las
  21 fotos originales que ya tenía incrustadas — nada se rompe. En cuanto
  subas al menos una foto nueva, esas pasan a ser las que se muestran.
...
nivel que `index.html`), junto con `api/admin.js` y toda la carpeta `lib/`
(contiene el código compartido: `_supabase.js`, `_mailer.js`,
`_calendar.js`, `_adminAuth.js`, `_refundPolicy.js`, `_trm.js`,
`_emailTemplates.js`, `_storage.js`, `_googleAuth.js`).
